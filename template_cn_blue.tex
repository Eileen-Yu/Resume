...
\cventry{2020.08-2020.10}{亚马逊}{数据分析实习生-AWS Infra}{}{}{\begin{itemize}
   \item[•] 参与AWS内部数据分析工作，输出标准化产出的基础上提升性能，并从中发现或预测问题。
   \item[-] 对150万条来源于不同渠道的原始数据进行清洗，过滤掉空值及异常值，并根据需求统一规范数据格式。
   \item[-] 使用AWS RDS进行数据库创建与管理；载入数据样本，初步对单表及多表间关系进行整合，建立指标体系；使用SQL语句大规模分析数据、查询结果。
   \item[-] 利用矩阵计算数据间线性关系，得出数据相关性；基于 k-均值聚类算法，构建数学模型，判断数据分类。
   \item[-] 数据可视化及分析报告撰写。
\end{itemize}}
